...
import { Clock, ExternalLink, ShieldAlert, ShieldCheck, RefreshCw } from "lucide-react"
	...
	return (
		<div className="bg-background px-4 py-3">
			...
			{loading ? (
				...
				<span className={`text-2xl font-bold tabular-nums font-mono ${valueClass ?? ""}`}>
					{value}
				</span>
			)}
		</div>
...
export default function LedgerPage() {
	...
	const blocked = transactions.filter((t) => t.status === "blocked" || t.status === "denied")
	const totalSpent = executed.reduce((sum, t) => sum + parseFloat(t.amount || "0"), 0)
		...
		<div className="space-y-6">
			...
			{initialLoading ? (
				...
				<div className="flex flex-col items-center py-24 text-center border border-dashed">
					<Clock className="size-8 text-muted-foreground/30 mb-3" strokeWidth={1} />
