...
import { Clock, Copy, ExternalLink, ScrollText, Shield, ShieldCheck } from "lucide-react"
	...
	useEffect(() => {
		...
		async function load() {
			try {
				...
				if (ledRes.status === "fulfilled") setTransactions(ledRes.value.transactions)
			} finally {
				...
				<Card className="rounded-none border">
					...
					<CardContent>
						{initialLoading ? (
							...
							<p className="text-2xl font-bold font-mono">${policy.dailyBudget}</p>
						) : (
							<div>
