...
const TABS = ["MCP", "REST API", "Agent Skill"] as const;
...
const CODE: Record<Tab, React.ReactNode> = {
  ...
  "REST API": (
    <div className="font-mono text-[11px] leading-[1.85]">
      <div>
        <Cm>
          {"// POST /pay — policy check runs before any funds move"}
        </Cm>
      </div>
      <div className="mt-1">
        <Kw>const </Kw>
...
const TAB_DESCRIPTIONS: Record<Tab, string> = {
  ...
  "REST API":
    "Call ZenithPay directly over HTTP. Every payment request is policy-checked and logged on X Layer before funds move.",
  "Agent Skill":
            ...
            <div className="mt-6">
              ...
              <div className="mt-2.5 flex flex-wrap gap-1.5">
                {[
                  ...
                  <span
                    key={name}
                    className="font-mono text-[10px] px-2 py-1 border border-border text-muted-foreground/70 bg-background"
                  >
                    {name}
                  </span>
                ))}
